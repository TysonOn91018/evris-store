(() => {
  ...
  let panel,message,button,last=null,user=null,busy=false;
  function render(){
  ...
  async function check(){
    if(!user || busy)return;busy=true;button.disabled=true;
    try {
  ...
  if(id && ['return','cancel'].includes(params.get('payment'))) {
    ...
    panel.append(message,button);document.querySelector('main').prepend(panel);
    window.EvrisBackend.auth.onAuthStateChange((_event,session)=>{user=session?.user || null;last=null;render();check();});
  }
